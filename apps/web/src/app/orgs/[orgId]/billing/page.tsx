...
export default function BillingPage() {
  ...
  return <>
    ...
    <div className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
      <Card>
        <div className="flex items-center justify-between"><div><Overline>Current plan</Overline><h2 className="mt-3 font-display text-2xl font-semibold">{titleCase(billing.plan)}</h2></div><PlanBadge plan={billing.plan} /></div>
      ...
      <Card><Overline>Plan-change history</Overline>{billing.history.length ? <div className="mt-4 divide-y divide-line">{billing.history.map((entry, index) => <div className="py-4 first:pt-0" key={`${entry.changedAt}-${index}`}><div className="flex items-center justify-between gap-4"><p className="font-mono text-[13px] text-ink">{titleCase(entry.fromPlan)} → {titleCase(entry.toPlan)}</p><time className="font-mono text-xs text-ink-faint">{formatDate(entry.changedAt)}</time></div><p className="mt-1 text-xs text-ink-soft">Changed by {entry.changedBy}</p></div>)}</div> : <div className="py-12 text-center"><p className="font-display text-lg">No plan changes yet</p><p className="mt-1 text-[13px] text-ink-soft">Updates will be recorded here.</p></div>}</Card>
    </div>
