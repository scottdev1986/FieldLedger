    ...
    <div className="mt-6 grid gap-4 lg:grid-cols-[2fr_1fr]">
      ...
      <div className="space-y-4">
        <Card>
          ...
          {currentSeason ? <><h2 className="mt-3 font-display text-xl font-medium">{currentSeason.name}</h2><div className="mt-5 space-y-4">{cropProgress.map((crop, index) => <div className="flex items-start gap-3" key={crop.crop}><span aria-hidden="true" className="mt-1.5 h-2 w-2 rounded-full" style={{ backgroundColor: `var(--color-chart-${(index % 6) + 1})` }} /><div className="min-w-0 flex-1"><div className="flex justify-between gap-3"><p className="font-medium">{crop.crop}</p><p className="font-mono text-xs text-ink-faint">{formatNumber(crop.acreage)} ac</p></div><p className="mt-1 text-xs text-ink-soft"><span className="font-mono">{crop.fieldCount}</span> {crop.fieldCount === 1 ? "field" : "fields"}{crop.daysToHarvest !== null ? <span> · <span className="font-mono">{crop.daysToHarvest}</span> days to harvest</span> : null}</p></div></div>)}</div></> : <div className="py-8 text-center"><Gauge className="mx-auto h-7 w-7 text-ink-faint" /><p className="mt-3 font-display text-lg">No current season</p>{canEdit ? <Button className="mt-4" onClick={() => setSeasonOpen(true)} size="sm">Create season</Button> : null}</div>}
        </Card>
