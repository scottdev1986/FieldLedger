...
export default function SeasonReportPage() {
  ...
  return <>
    <div className="no-print mb-5 flex flex-wrap items-end justify-between gap-4"><div><p className="overline">{org.name}</p><h1 className="mt-2 text-2xl font-semibold tracking-tight">Season report</h1></div><div className="flex gap-2">{org.role !== "viewer" ? <Button loading={exporting} onClick={() => void download()} variant="secondary"><Download className="h-4 w-4" />Export CSV</Button> : null}<Button onClick={() => window.print()}><Printer className="h-4 w-4" />Print / Save PDF</Button></div></div>
    <article className="report-page mx-auto max-w-[800px] border border-line bg-white p-8 shadow-card sm:p-12">
      <header className="border-b border-line pb-8"><div className="flex items-start justify-between gap-4"><div><Overline>FieldLedger season record</Overline><h2 className="mt-3 text-3xl font-semibold tracking-tight">{report.organization.name}</h2><p className="mt-2 text-lg font-medium text-ink-soft">{report.season.name}</p></div><PlanBadge plan={report.organization.plan} /></div><p className="mt-7 font-mono text-xs text-ink-faint">Generated {formatDate(report.generatedAt, { month: "long", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" })}</p></header>
